...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		*/
		...
		var vgblack = new FlxSprite().loadGraphic(Paths.image("hud/base/bars"));
		vgblack.alpha = 1;
		...
		drain = new FlxSprite().loadGraphic(Paths.image("hud/spells/spell1"));
		drain.updateHitbox();
		drain.y = 490;
		drain.screenCenter(X);
		drain.cameras = [camHUD];
		drain.alpha = 0;
		add(drain);

		speed = new FlxSprite().loadGraphic(Paths.image("hud/spells/spell2"));
		speed.updateHitbox();
		speed.y = 490;
		speed.screenCenter(X);
		speed.cameras = [camHUD];
		speed.alpha = 0;
		add(speed);

		flip = new FlxSprite().loadGraphic(Paths.image("hud/spells/spell3"));
		flip.updateHitbox();
		flip.y = 490;
		flip.screenCenter(X);
		flip.cameras = [camHUD];
		...
		bombs = new FlxSprite().loadGraphic(Paths.image("hud/spells/spell4"));
		bombs.updateHitbox();
		bombs.y = 490;
		bombs.screenCenter(X);
		bombs.cameras = [camHUD];
		bombs.alpha = 0;
		add(bombs);
	...
	function onNoteHit(note:Note, strumline:Strumline)
	{
		...
		if(thisChar != null && !note.isHold)
		{
			if(note.noteType != "no animation")
			{
				...
				if(healthDrain && !strumline.isPlayer) {
					if (health > 0.2)
					{
						if (note.isHold || note.isHoldEnd)
						{
	...
	function onNoteMiss(note:Note, strumline:Strumline)
	{
		...
		if(onlyOnce)
		{
			if(note.noteType == "bomb") {
				strumline.playSplash(note);
				FlxG.sound.play(Paths.sound('beep'), 1);
				health -= 0.25;
			}
			else {
				vocals.volume = 0;
	...
	var cameraMoveItensity:Float = 24;
	...
	var banList:Array<String> = ['spell'];
	...
	override function stepHit()
	{
		...
		switch(curStep) {
			case 453:
				dad.playAnim("spell", true);

				FlxTween.tween(bombs, {alpha: 1}, 0.4, {
					ease: FlxEase.cubeOut,
					onComplete: function(twn:FlxTween)
					{
						FlxTween.tween(bombs, {alpha: 0}, 0.6, {
							ease: FlxEase.cubeIn,
							startDelay: 2
						});
					}
			...
			case 464:
				CoolUtil.flash(camStrum, 0.6);
				defaultCamZoom = 0.65;
			
			case 848: //848
				CoolUtil.flash(camStrum, 0.6);

				setScroll(!SaveData.data.get('Downscroll'));

				FlxTween.tween(flip, {alpha: 1}, 0.01, {
					ease: FlxEase.cubeOut,
					onComplete: function(twn:FlxTween)
					{
						FlxTween.tween(flip, {alpha: 0}, 0.6, {
							ease: FlxEase.cubeIn,
							startDelay: 2
						});
					}
			...
			case 1364:
				...
				FlxTween.tween(drain, {alpha: 1}, 0.4, {
					ease: FlxEase.cubeOut,
					onComplete: function(twn:FlxTween)
					{
						FlxTween.tween(drain, {alpha: 0}, 0.6, {
							ease: FlxEase.cubeIn,
							startDelay: 2
						});
					}
			...
			case 2012:
				for (strum in strumlines)
					FlxTween.tween(strum, {scrollSpeed: 3.7}, 1.5, {ease: FlxEase.linear});

				FlxTween.tween(speed, {alpha: 1}, 0.4, {
					ease: FlxEase.cubeOut,
					onComplete: function(twn:FlxTween)
					{
						FlxTween.tween(speed, {alpha: 0}, 0.6, {
							ease: FlxEase.cubeIn,
							startDelay: 2
						});
					}
			...
			case 2144:
				...
				for (strum in strumlines)
					FlxTween.tween(strum, {scrollSpeed: 3.1}, 0.01, {ease: FlxEase.linear});
			case 2544:
				CoolUtil.flash(camOther, 0.6);
				hudBuild.setAlpha(0);
				camStrum.alpha = 0;
	...
	public function endSong()
	{
		...
		CoolUtil.playMusic();
		Main.switchState(new states.story.GoodEnding());
